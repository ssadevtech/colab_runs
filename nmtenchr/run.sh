...
if [ "$1" = "train" ]; then
    ...
    CUDA_VISIBLE_DEVICES=0 python run_nmt.py decode model.bin ./data/test.chr ./data/test.en outputs.txt --spmmodel-src=./data/src.model --spmmodel-tgt=./data/tgt.model --cuda --tok=spm
else
    echo "Invalid option"
fi
