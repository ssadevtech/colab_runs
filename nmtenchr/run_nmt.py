...
Options:
    ...
    --vocab-size=<int>                      vocab size in case nltk tok used [Default: 50000]
    --vocab-cutoff=<int>                    Cutoff in case nltk tok used [Default: 2]
    ...
    --tok=<str>                             Tokenizer (spm or nltk) [Default: nltk]
    --spmmodel-src=<file>                   Path to the src spm model [Default: ./spm/src.model]
    --spmmodel-tgt=<file>                   Path to the tgt spm model [Default: ./spm/tgt.model]
    --subset=<float>                        Percentage of data to consider [Default: 1]
    --random-subset=<bool>                  Generate a random subset [Default: 0]
"""
import math
import sys
import pickle
...
from nmt_utils import read_corpus, batch_iter, read_corpus_spm
...
def compute_corpus_level_bleu_score(references: List[List[str]], hypotheses: List[Hypothesis], tokenizer:str ='nltk') -> float:
    ...
    if tokenizer == 'nltk':
        ...
        raise Exception(f'unrecognised tokenizer {tokenizer}. Should be nltk or spm')
...
def load_data_train(train_src_path, train_tgt_path, dev_src_path, dev_tgt_path, tokenizer='nltk', spm_model_src='./spm/src.model', 
                    spm_model_tgt ='./spm/tgt.model',create_vocab=False, vocab_path=None, vocab_size=50000, vocab_cutoff = 2, 
                    subset=1.0, random_subset=False):
    ...
    if tokenizer == 'nltk':
        ...
        train_data_src = read_corpus_spm(train_src_path, corpus_type='src', model_path=spm_model_src)
        train_data_tgt = read_corpus_spm(train_tgt_path, corpus_type='tgt', model_path=spm_model_tgt)
        dev_data_src = read_corpus_spm(dev_src_path, corpus_type='src', model_path=spm_model_src)
        dev_data_tgt = read_corpus_spm(dev_tgt_path, corpus_type='tgt', model_path=spm_model_tgt)

    else:
        raise ValueError(f'Incorrect value [{tokenizer}] for tokenizer; should be nltk or spm')
    ...
    if create_vocab and tokenizer == 'nltk':
        ...
        if vocab_path is not None:
            ...
            raise ValueError(f'Incorrect combination {tokenizer}, {create_vocab}, {vocab_path}')
...
def decode(test_src_path, test_tgt_path=None, model_path='model.bin', tokenizer='nltk', spm_model_src='./spm/src.model', 
                    spm_model_tgt ='./spm/tgt.model', beam_size=5, max_decoding=70, device='cpu', output_path='output.txt'):
    ...
    if tokenizer == 'nltk':
        ...
        test_data_src = read_corpus_spm(test_src_path, corpus_type='src', model_path=spm_model_src)
    else:
        raise Exception(f'unrecognised tokenizer {tokenizer}. Should be nltk or spm')
    ...
    if test_tgt_path is not None:
        ...
        if tokenizer == 'nltk':
            ...
            test_data_tgt = read_corpus_spm(test_tgt_path, corpus_type='tgt', model_path=spm_model_tgt)
...
def main():
    ...
    tok = args['--tok']
    spm_model_src = args['--spmmodel-src'] if tok == 'spm' else None
    spm_model_tgt = args['--spmmodel-tgt'] if tok == 'spm' else None
    ...
    if args['train']:
        ...
        vocab_cutoff = int(args['--vocab-cutoff'])
        subset = float(args['--subset'])
        random_subset = bool(args['--random-subset'])

        train_data, dev_data, vocab = load_data_train(train_src_path, train_tgt_path, dev_src_path, dev_tgt_path, tokenizer=tok,
                                                      spm_model_src=spm_model_src, spm_model_tgt=spm_model_tgt, create_vocab=False, 
                                                      vocab_path=vocab_path, vocab_size=vocab_size, vocab_cutoff = vocab_cutoff,
                                                      subset=subset, random_subset=random_subset)
        ...
        decode(test_src_path=test_src_path, test_tgt_path=test_tgt_path, model_path=model_path, tokenizer=tok, 
               spm_model_src=spm_model_src, spm_model_tgt=spm_model_tgt, beam_size = beam_size, max_decoding=max_decoding, 
