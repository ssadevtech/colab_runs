...
Options:
    ...
    --model-path=<file>        Path for tokenization model (with spm tokenizer) 
"""
...
def get_vocab_list(file_path, corpus_type, vocab_size, model_path=None):
    ...
    """
    model_path = f'{corpus_type}' if model_path is None else f'{model_path}/{corpus_type}'
    # train the spm model 
    spm.SentencePieceTrainer.train(input=file_path, model_prefix=model_path, vocab_size=vocab_size) 
...
if __name__ == '__main__':
    ...
    if args["--tok"] == 'nltk':
        ...
        print(f'read in source sentences: {args["--src"]}...')
        src_sents = get_vocab_list(args['--src'], corpus_type='src', vocab_size=args["--src-size"], model_path=args["--model-path"])  
        
        print(f'read in target sentences: {args["--tgt"]}...')
        tgt_sents = get_vocab_list(args['--tgt'], corpus_type='tgt', vocab_size=args["--tgt-size"], model_path=args["--model-path"])
